...
def escape_js_string(s: str):
    # Embed raw JSON in a <script type="application/json"> block.
    # Keep JSON valid and only guard against accidental </script> termination.
    return s.replace("</", "<\/")
...
def main():
    ...
    html_txt, replacements = _re.subn(
        r'(<script\s+id="hne-data"\s+type="application/json">)(.*?)(</script>)',
        lambda m: m.group(1) + new_json + m.group(3),
        html_txt,
        flags=_re.DOTALL,
    )
    if replacements != 1:
        raise SystemExit('Could not locate exactly one <script id="hne-data" type="application/json"> block in hne_viewer.html')
